...
export default function IdealPostureAngel() {
  ...
  return (
    <div style={{ padding: "48px", fontFamily: "sans-serif" }}>
      <h1>A. Ideal Posture Angle</h1>
      <p style={{ color: "#888", marginTop: "16px" }}>
        (Konten akan ditambahkan nanti)
      </p>

      <div style={{ display: "flex", gap: "12px", marginTop: "48px" }}>
        <button onClick={() => navigate("/impact-of-bad-posture")} style={btnStyle("white", "black")}>
          ← Back
        </button>
        <button onClick={() => navigate("/")} style={btnStyle("white", "black")}>
          Home
        </button>
        <button onClick={() => navigate("/guide-for-posture-check")} style={btnStyle("black", "white")}>
          Next →
        </button>
      </div>
    </div>
